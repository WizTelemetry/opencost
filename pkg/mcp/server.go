...
type EfficiencyResponse struct {
	Efficiencies []*costmodel.EfficiencyMetric `json:"efficiencies"`
}
...
func (s *MCPServer) QueryEfficiency(query *OpenCostQueryRequest) (*EfficiencyResponse, error) {
	...
	var aggregateBy []string
	var filterString string
	bufferMultiplier := costmodel.DefaultEfficiencyBufferMultiplier
	...
	// 4. Call shared efficiency computation
	metrics, err := costmodel.ComputeEfficiency(s.costModel, window, aggregateBy, filterString, bufferMultiplier)
	if err != nil {
		return nil, fmt.Errorf("failed to compute efficiency: %w", err)
	}
